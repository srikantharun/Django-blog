...
class Command(BaseCommand):
    help = 'Creates a blog post about SSLV geometry representation using Florence2 model'

    def handle(self, *args, **kwargs):
        # Create or get AI & Computer Vision category
        category, created = Category.objects.get_or_create(
            name="AI & Computer Vision",
            defaults={
                'slug': 'ai-computer-vision',
                'description': 'Articles about artificial intelligence, computer vision, and machine learning applications'
            }
        )
        ...
        # Create or get Space Technology category
        space_category, created = Category.objects.get_or_create(
            name="Space Technology",
            defaults={
                'slug': 'space-technology',
                'description': 'Articles about space technology, rockets, satellites, and related innovations'
            }
        )
        
        if created:
            self.stdout.write(self.style.SUCCESS(f'Created new category: {space_category.name}'))
        else:
            self.stdout.write(self.style.SUCCESS(f'Using existing category: {space_category.name}'))
        ...
        try:
            ...
            admin_user = User.objects.create_superuser(
                ...
                password='adminpassword'  # You should change this in a production environment
            )
            self.stdout.write(self.style.SUCCESS('Created admin user'))

        # Content for the blog post
        title = "Using Florence2 model to create SSLV geometry representation"
...
"""
        ...
        try:
            ...
            try:
                ...
                self.stdout.write(self.style.WARNING(f'Post with title "{title}" already exists, updating content.'))
                ...
                post.categories.add(space_category)
                self.stdout.write(self.style.SUCCESS(f'Created new blog post: {post.title}'))
                
            self.stdout.write(self.style.SUCCESS(f'Blog post available at: {post.get_absolute_url()}'))
            
        except Exception as e:
            self.stdout.write(self.style.ERROR(f'Error creating blog post: {e}'))