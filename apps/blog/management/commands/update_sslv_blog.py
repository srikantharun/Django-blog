...
class Command(BaseCommand):
    help = 'Updates the SSLV blog post with improved formatting and images'

    def handle(self, *args, **kwargs):
        try:
            post = Post.objects.get(title='Using Florence2 model to create SSLV geometry representation')
            
            # Updated blog content with proper image references
            blog_content = """# Harnessing Florence-2 for Spacetech: Multimodal Analysis of SSLV Designs
...
![DaViT Architecture](/media/blog_images/florence2/davit_arch.svg)
...
![Florence-2 Model Architecture 1](/media/blog_images/florence2/florence_model1.svg)

![Florence-2 Model Architecture 2](/media/blog_images/florence2/florence_model2.svg)
...
### 1. Use StableDiffusion pipeline to generate image based on prompt
...
### 2. Helper Function To Run Florence-2 Tasks
...
### 3. Image Captioning for Component Documentation
...
### 4. Object Detection for Assembly Verification
...
![Object Detection on SSLV](/media/blog_images/florence2/object_detection.svg)
...
### 5. Segmentation for Quality Control
...
![Segmentation of SSLV components](/media/blog_images/florence2/segmentation.svg)
...
### 6. Visual Question Answering (VQA) for Design Consultation
...
![Visual Question Answering with Florence-2](/media/blog_images/florence2/vqa.svg)
...
"""
            ...
            self.stdout.write(self.style.SUCCESS(f'Blog post "{post.title}" has been updated with improved formatting and images'))
            
        except Post.DoesNotExist:
            self.stdout.write(self.style.ERROR(f'Blog post not found'))
            return